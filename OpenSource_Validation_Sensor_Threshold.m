...
start = 1; %update as needed
endfile = 14; %update as needed
...
%    
        ...
        %Designing lowpass Butterworth filter at the frequency defined
        %above
...
%Calculate user acceleration from filtered data
  ...
  %%Find the index where the median is present
   medI = find(A_total_p_filt(:,3) == median(A_total_p_filt(:,3)),1, 'first');
    ...
    %if median is not within the dataset, you need to find the index of the
    %closest value with this method and comment out line previous line (most common with shorter periods of data) 
...
%     medI = idx(1);
 ...
 %Calculate acceleration magnitudes
  ...
  %magnitude %filtered without threshold applied
    ...
    legend([h4u],['A_{user filtered}']); %{_makes it subscript}
    xlabel('Time (sec)');
    ylabel('Acceleration (m/s/s)'); 
    xlim([0,1900]);
     ...
     Time_interval = time_zero(2)-time_zero(1);
          %% 8 sec interval 
%   Eightsecinterval = floor(8/Time_interval); %data points within both timezero and A_user_mag_filt
%     for u = 1: (length(time_zero) - Eightsecinterval)
%         start_interval(u) = u;
%         stop_interval(u) = u + Eightsecinterval;
...
%     end
     %% 10 sec interval 
%   Tensecinterval = floor(10/Time_interval); %data points within both timezero and A_user_mag_filt
%     for u = 1: (length(time_zero) - Tensecinterval)
%         start_interval(u) = u;
%         stop_interval(u) = u + Tensecinterval;
%         output_mean(u) = mean(A_user_mag_filt(start_interval(u):stop_interval(u),:));
%         output_std(u) = std(A_user_mag_filt(start_interval(u):stop_interval(u),:));
%     end
   %%  15 sec interval
     Fifteensecinterval = floor(15/Time_interval); %data points within both timezero and A_user_mag_filt
   for u = 1: (length(time_zero) - Fifteensecinterval)
       start_interval(u) = u;
       stop_interval(u) = u + Fifteensecinterval;
       output_mean(u) = mean(A_user_mag_filt(start_interval(u):stop_interval(u),:));
       output_std(u) = std(A_user_mag_filt(start_interval(u):stop_interval(u),:));
   end
%     
      %%  30 sec interval
%     Thirtysecinterval = floor(30/Time_interval); %data points within both timezero and A_user_mag_filt
%    for u = 1: (length(time_zero) - Thirtysecinterval)
%        start_interval(u) = u;
%        stop_interval(u) = u + Thirtysecinterval;
%        output_mean(u) = mean(A_user_mag_filt(start_interval(u):stop_interval(u),:));
%        output_std(u) = std(A_user_mag_filt(start_interval(u):stop_interval(u),:));
%    end
...
Interval_25 = floor((length(time_zero))/4);
   for u = 1:Interval_25 - Fifteensecinterval
       start_interval_1_25(u) = u;
       stop_interval_1_25(u) = u + Fifteensecinterval;
...
%26-50%
   for u =(Interval_25 + 1) : (Interval_25 * 2 - Fifteensecinterval)
       start_interval_26_50(u-Interval_25) = u;
       stop_interval_26_50(u-Interval_25) = u + Fifteensecinterval;
...
%51-75%
   for u = (Interval_25*2 + 1): (Interval_25 *3 - Fifteensecinterval)
       start_interval_51_75(u-(Interval_25*2)) = u;
       stop_interval_51_75(u-(Interval_25*2)) = u + Fifteensecinterval;
...
Upper_threshold_51_75 = Mean_A_user_mag_filt_51_75 + (3*STD_A_user_mag_filt_51_75); %modify  here if changing thresho

%76-100%
   for u = ((Interval_25*3)+1): ((Interval_25 *4) - Fifteensecinterval)
       start_interval_76_100(u-(Interval_25*3)) = u;
       stop_interval_76_100(u-(Interval_25*3)) = u + Fifteensecinterval;
...
Upper_threshold_76_100 = Mean_A_user_mag_filt_76_100 + (3*STD_A_user_mag_filt_76_100); %modify  here if changing thresho
...
%ID index of stop and start of active times. (by subracting values to determine a "change" indicating start or stop) 
%All other points will be "0"
    for o = 2: length(ext_time_threshold) - 1
        if ext_time_threshold(o) - ext_time_threshold(o-1) < 0
            idx_stop_active_sensor(o-1) = o-1;
        elseif (round(ext_time_threshold(o) - ext_time_threshold(o-1)) > Time_interval)
            idx_start_active_sensor(o) = o;
        end
    end

% % %removes "0" to be left with only stop/start of active times, should be
% equal in length
idx_start_active_sensor(idx_start_active_sensor==0) = [];
idx_stop_active_sensor(idx_stop_active_sensor==0) = [];
%rows to column for readability
idx_start_active_sensor = idx_start_active_sensor';
idx_stop_active_sensor = idx_stop_active_sensor';
...
%create active time windows
 for j = 1:length(active_time_video)
start_active_time(j) = active_time_video(j, 5) - video_time_in_place; %
stop_active_time(j) = active_time_video(j,6) - video_time_in_place;
active_time_windows(j) = stop_active_time(j) - start_active_time(j);
 end
 
 %create picked up/removed time windows
 size_array = size(picked_up_removed_time_video);
 for Compare_threshold_video = 1:size_array(1)
start_picked_up_removed_time(Compare_threshold_video) = picked_up_removed_time_video(Compare_threshold_video, 5) - video_time_in_place;
stop_picked_up_removed_time(Compare_threshold_video) = picked_up_removed_time_video(Compare_threshold_video,6) - video_time_in_place;
picked_up_removed_time_windows(Compare_threshold_video) = stop_picked_up_removed_time(Compare_threshold_video) - start_picked_up_removed_time(Compare_threshold_video);
 end
...
for m = 1:length(time_video_active_alt)
if time_video_active_alt(m)~= 2 && time_threshold_alt(m) == 2
    false_pos_direct(m) = 1;
else
    false_pos_direct(m) = 0;
end 
end 

for m = 1:length(time_video_active_alt)
if time_video_active_alt(m)== 2 && time_threshold_alt(m) ~= 2
    false_neg_direct(m) = 1;
else
    false_neg_direct(m) = 0;
end
end
...
for p = 1: length(data_start_idx)
    ...
    % Makes a figure for each active movement bout-- this will crash the
    % program, only use if really needed, or pull one out. 
...
%Another way to get the active bouts, without a loop. Not used. 
% % % for p = 1: length(data_start_idx)
% % %     active_video_bout(data_start_idx(p):data_end_idx(p)) = active_video_A_user_mag_filt(data_start_idx(p):data_end_idx(p));
% % % end
% % % 
% % % time_video_active(data_start_idx(n):data_end_idx(n))= time_zero(data_start_idx(n):data_end_idx(n)); 
% % %     active_video_A_user_mag_filt(data_start_idx(n):data_end_idx(n)) = A_user_mag_filt(data_start_idx(n):data_end_idx(n));
...
%Replace 0 in Time_threshold with -100 for comparison between files
for v = 2:length(time_threshold)
    if time_threshold(v) == 0
        time_threshold(v) = NaN;     
    else
        time_threshold(v) = time_threshold(v);
    end
end  
